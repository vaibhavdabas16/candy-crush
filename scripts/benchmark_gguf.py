"""Benchmark any GGUF Qwen-arch model on the Candy Crush eval boards.

Same protocol as benchmark_openai_vs_grpo.py (10 fixed boards, 20-move
rollouts, no_fallback) but works for an arbitrary local GGUF file -
useful for comparing the GRPO-trained merge against the un-modified
base model.
"""
...
def play(agent, label: str, seed: int, special_seed: int, max_moves: int) -> dict:
    ...
    for _ in range(max_moves):
        ...
        if terminated or truncated:
            break
    print(
        f"  [{label:>20s}] seed={seed} reward={total_reward:8.2f} "
...
def main() -> None:
    args = parse_args()
    label = args.label or Path(args.gguf_path).stem
    print(f"GGUF benchmark: {label}  seeds={args.seeds} max_moves={args.max_moves}")
    ...
    per_seed = [play(agent, label, s, s + args.special_offset, args.max_moves) for s in args.seeds]
    rewards = [r["reward"] for r in per_seed]
    avg, std = float(np.mean(rewards)), float(np.std(rewards, ddof=0))
    print()
    print("=" * 72)
    print(f"Summary  policy={label}")
    print("=" * 72)
